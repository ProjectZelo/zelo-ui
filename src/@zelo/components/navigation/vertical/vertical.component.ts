...
import { ZeloNavigationItem, ZeloVerticalNavigationAppearance, ZeloVerticalNavigationMode, ZeloVerticalNavigationPosition } from '@Zelo/components/navigation/navigation.types';
...
@Component({
    selector: 'zelo-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: zeloAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'zeloVerticalNavigation'
})
export class ZeloVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    @Input() appearance: ZeloVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: ZeloVerticalNavigationMode = 'side';
    @Input() name: string = this._zeloUtilsService.randomId();
    @Input() navigation: ZeloNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: ZeloVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<ZeloVerticalNavigationAppearance> = new EventEmitter<ZeloVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<ZeloVerticalNavigationMode> = new EventEmitter<ZeloVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<ZeloVerticalNavigationPosition> = new EventEmitter<ZeloVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<ZeloNavigationItem> = new ReplaySubject<ZeloNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<ZeloNavigationItem> = new ReplaySubject<ZeloNavigationItem>(1);
    ...
    private _zeloScrollbarDirectives!: QueryList<ZeloScrollbarDirective>;
    private _zeloScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        return {
            'zelo-vertical-navigation-animations-enabled': this._animationsEnabled,
            [`zelo-vertical-navigation-appearance-${this.appearance}`]: true,
            'zelo-vertical-navigation-hover': this._hovered,
            'zelo-vertical-navigation-inner': this.inner,
            'zelo-vertical-navigation-mode-over': this.mode === 'over',
            'zelo-vertical-navigation-mode-side': this.mode === 'side',
            'zelo-vertical-navigation-opened': this.opened,
            'zelo-vertical-navigation-position-left': this.position === 'left',
            'zelo-vertical-navigation-position-right': this.position === 'right'
        };
    }
    ...
    /**
     * Setter for zeloScrollbarDirectives
     */
    @ViewChildren(ZeloScrollbarDirective)
    set zeloScrollbarDirectives(zeloScrollbarDirectives: QueryList<ZeloScrollbarDirective>) {
        // Store the directives
        this._zeloScrollbarDirectives = zeloScrollbarDirectives;

        // Return if there are no directives
        if (zeloScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._zeloScrollbarDirectivesSubscription) {
            this._zeloScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._zeloScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    zeloScrollbarDirectives.forEach((zeloScrollbarDirective) => {
                        zeloScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            ...
            if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.zelo-vertical-navigation-item-active');
            ...
            // Otherwise
                ...
                this._zeloScrollbarDirectives.forEach((zeloScrollbarDirective) => {

                    // Skip if not enabled
                    if (!zeloScrollbarDirective.isEnabled()) {
                        return;
                    }

                    // Scroll to the active element
                    zeloScrollbarDirective.scrollToElement('.zelo-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: ZeloNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: ZeloNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('zelo-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add('zelo-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add('zelo-vertical-navigation-aside-overlay');
