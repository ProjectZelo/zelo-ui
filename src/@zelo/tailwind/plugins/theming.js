...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity = (name) => ({
        ...
        if (opacityValue) {
            return `rgba(var(--zelo-${name}-rgb), ${opacityValue})`;
        }
        if (opacityVariable) {
            return `rgba(var(--zelo-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--zelo-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--zelo-${e(key)}`, value], [`--zelo-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
    ]));
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
        ...
        const background = theme(`zelo.customProps.background.${colorScheme}`);
        const foreground = theme(`zelo.customProps.foreground.${colorScheme}`);
        ...
        return {
            [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                ...
                ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--zelo-${e(key)}`, value], [`--zelo-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--zelo-${e(key)}`, value], [`--zelo-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            }
        };
    });
